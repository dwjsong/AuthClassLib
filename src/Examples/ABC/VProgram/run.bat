...
SET STUB=stub
SET file_name=bin\Release\vProgram
...
copy %POIROT_ROOT%\poirot4.net\library\poirot_stubs.bpl
 ...
 C:\Windows\Microsoft.NET\Framework64\v4.0.30319\MSBuild.exe vProgram.csproj /p:OutputPath=bin\Release /p:Configuration=Release

call %POIROT_ROOT%\Poirot4.NET\BCT\BytecodeTranslator.exe /e:1 /ib /whole /heap:splitFields %file_name%.exe bin\Release\Server_A.dll bin\Release\Server_B.dll bin\Release\Server_C.dll bin\Release\ABC_Common.dll bin\Release\CST_Common.dll  
